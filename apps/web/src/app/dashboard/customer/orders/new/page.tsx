            ...
            {order.shopId && (
              <section>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {loadingServices ? (
                    <p className="col-span-2 text-sm text-neutral-400 text-center py-4">{isRTL ? 'جاري التحميل...' : 'Loading...'}</p>
                  ) : services.length === 0 ? (
                    <p className="col-span-2 text-sm text-amber-600 text-center py-4">{isRTL ? 'لا توجد خدمات لهذا المتجر' : 'No services for this shop'}</p>
                  ) : services.map((svc) => (
                    <button
                      key={svc.id}
                      type="button"
