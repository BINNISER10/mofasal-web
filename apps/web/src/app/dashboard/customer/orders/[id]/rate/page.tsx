...
import {
  Star, ChevronLeft, ChevronRight, Send, ArrowLeft,
} from 'lucide-react';
...
export default function RateOrderPage() {
  ...
  const handleSubmit = async () => {
    if (shopRating === 0) {
      toast.error(isRTL ? 'يرجى تقييم المتجر' : 'Please rate the shop');
      return;
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-[70vh] p-6">
        <div className="text-center max-w-md mx-auto">
          <div className="w-24 h-24 rounded-full bg-[#00373E]/10 flex items-center justify-center mx-auto mb-6">
            <Star size={48} className="text-[#D4AF37] fill-[#D4AF37]" />
          </div>
          <h1 className="text-2xl font-black text-[#00373E] mb-3">
            {isRTL ? 'شكراً لتقييمك!' : 'Thank You!'}
          </h1>
          <p className="text-[#735B4D]/60 mb-6">
            {isRTL ? 'تقييمك يساعدنا على تحسين الخدمة' : 'Your review helps us improve our service'}
          </p>
          <Button variant="primary" onClick={() => router.push('/dashboard/customer/orders')}>
            {isRTL ? 'العودة لطلباتي' : 'Back to Orders'}
          </Button>
        </div>
      </div>
    );
      ...
      <div className="flex items-center gap-1">
        {[1, 2, 3, 4, 5].map((n) => (
          ...
          >
            <Star
              size={28}
              className={n <= rating ? 'text-[#D4AF37] fill-[#D4AF37]' : 'text-[#D0D6D7]'}
            />
          </button>
        ))}
        {rating > 0 && (
          <span className="text-sm font-bold text-[#00373E] ms-2">{rating}/5</span>
        )}
      </div>
    </div>
  );
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <Card className="p-6 space-y-6">
        <div className="text-center mb-4">
          <Star size={40} className="text-[#D4AF37] mx-auto mb-2" />
          <h2 className="text-lg font-bold text-[#00373E]">
            {isRTL ? 'كيف كانت تجربتك؟' : 'How was your experience?'}
          </h2>
          <p className="text-sm text-[#735B4D]/60">
            {isRTL ? 'تقييمك يساعد العملاء الآخرين' : 'Your review helps other customers'}
          </p>
        </div>
