...
export default function AdminDashboardPage() {
  ...
  const stats = [
    { icon: <Users size={22} />, label: isRTL ? 'إجمالي المستخدمين' : 'Total Users', value: dashboardData?.totalUsers?.toLocaleString() || '0', trend: 12.5, color: 'primary' as const, href: '/dashboard/admin/users' },
    { icon: <Store size={22} />, label: isRTL ? 'إجمالي المتاجر' : 'Total Shops', value: dashboardData?.totalShops?.toLocaleString() || '0', trend: 8.3, color: 'gold' as const, href: '/dashboard/admin/shops' },
    { icon: <Package size={22} />, label: isRTL ? 'إجمالي التجار' : 'Total Merchants', value: dashboardData?.totalMerchants?.toLocaleString() || '0', trend: 5.2, color: 'info' as const, href: '/dashboard/admin/merchants' },
    { icon: <ShoppingBag size={22} />, label: isRTL ? 'إجمالي الطلبات' : 'Total Orders', value: dashboardData?.totalOrders?.toLocaleString() || '0', trend: 15.7, color: 'success' as const, href: '/dashboard/admin/orders' },
    { icon: <DollarSign size={22} />, label: isRTL ? 'الإيرادات (ريال)' : 'Revenue (SAR)', value: dashboardData?.totalRevenue?.toLocaleString() || '0', trend: 22.4, color: 'primary' as const, href: '/dashboard/admin/reports' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Link href="/dashboard/admin/analytics" className="block group">
          <Card className="p-5 transition-colors group-hover:border-[#00373E]/30 group-hover:shadow-sm cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/admin/orders" className="text-sm text-primary-700 hover:text-primary-800 font-semibold">
              ...
              <tbody>
                {recentOrders.map((order: any) => (
                  <tr
                    ...
                    className="border-b border-gray-50 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/50 dark:text-slate-300 cursor-pointer transition-colors"
                  >
                    <td className="py-2.5 px-2 font-medium text-primary-700">#{order.orderNumber || order.id?.slice(0, 8)}</td>
                    <td className="py-2.5 px-2">{order.customerName || order.customer?.name || order.customer}</td>
                    <td className="py-2.5 px-2">{order.shopName || order.shop?.name || order.shop}</td>
                    <td className="py-2.5 px-2">
                      <Badge variant={order.status === 'DELIVERED' ? 'success' : order.status === 'PENDING' ? 'warning' : order.status === 'ON_WAY_TO_CUSTOMER' ? 'info' : 'info'} size="sm">
                        {isRTL ? ({ DELIVERED: 'تم التسليم', ON_WAY_TO_CUSTOMER: 'في الطريق', PENDING: 'قيد الانتظار', CONFIRMED: 'تم التأكيد', SEWING_ASSEMBLY: 'خياطة' } as Record<string, string>)[order.status] || order.status : order.status}
                      </Badge>
                    </td>
        ...
        <div className="space-y-6">
          <Card className="p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/dashboard/admin/users" className="text-xs text-primary-700 font-semibold">
                {isRTL ? 'عرض الكل' : 'View All'}
              </Link>
            </div>
            ...
            <div className="space-y-3">
              {recentUsers.map((user: any, i: number) => (
                <Link
                  ...
                  className="flex items-center justify-between py-1.5 -mx-2 px-2 rounded-lg hover:bg-[#FAFAFA] dark:hover:bg-white/5 transition-colors"
                >
                  <div>
              ...
              <Link href="/dashboard/admin/users" className="block py-4 text-center text-primary-700 hover:underline text-sm">
                {isRTL ? 'عرض كل المستخدمين ←' : 'View all users →'}
              </Link>
            )}
          </Card>
          ...
          <Link href="/dashboard/admin/security" className="block group">
            <Card className="p-5 transition-colors group-hover:border-[#00373E]/30 group-hover:shadow-sm cursor-pointer">
              ...
              <p className="text-xs text-primary-700 mt-4 font-medium">{isRTL ? 'عرض الأمان والمراقبة ←' : 'View security & monitoring →'}</p>
            </Card>
          </Link>
        </div>
      </div>
