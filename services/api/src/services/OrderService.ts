...
export class OrderService {
  ...
  static async createOrder(data: {
    ...
    fabricId?: string; fabricSource?: string;
    measurements?: Record<string, number>;
    deliveryAddress?: { label?: string; street: string; district?: string; city: string };
    items?: Array<{ name: string; quantity: number; unitPrice: number }>;
  }) {
    const orderNumber = this.generateOrderNumber();
    const itemsTotal = data.items?.reduce((sum, i) => sum + i.quantity * i.unitPrice, 0) ?? 0;
    const totalAmount = data.totalAmount ?? itemsTotal;
    const deliveryFee = data.deliveryFee || 0;
    const vatAmount = totalAmount * 0.15;
    const grandTotal = totalAmount + vatAmount + deliveryFee;

    let notes = data.customerNotes || '';
    if (data.fabricSource) {
      notes = `${notes}\nمصدر القماش: ${data.fabricSource === 'marketplace' ? 'سوق الأقمشة' : 'المتجر'}`.trim();
    }
